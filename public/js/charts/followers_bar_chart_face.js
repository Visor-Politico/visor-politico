...
Highcharts.chart('followers_bar_chart_face', {
    chart: {
            type: 'column',
            marginTop: 60,
            borderColor: '#3b5998',
    ...
    xAxis: {
        ...
        labels: {
            enabled: true,
            formatter: function() { return this.value.capitalize() + '<br>' + jsonData2[this.value]["data"]["facebook"]; }
        }
    },
    yAxis: {
            title: {
    ...
    tooltip: {
        ...
        pointFormatter: function() {
            var tw = "";
            if (!jsonData2[this.name]["data"].facebook) {
                tw = "(Sin Facebook)";
            }
            return '<span style="font-size:11px; color:'+this.color+'">'+this.name.capitalize()+','+ jsonData2[this.name]["data"].actor_politico+': </span> <br> <b>'+thousandsSep(this.y)+'</b> seguidores '+tw+'<br/>'; },
        borderRadius: 5,
        hideDelay: 0
    },
    series: [{
...
function filterData() {
    candidates = [];
    for (var candidate in jsonData2) {
        _formatter.push(jsonData2[candidate]["data"].picture);
        candidates.push({
            "name": candidate,
            "y": jsonData2[candidate]["data"].followers,
            "dataLabels" : {
                enabled: true,
                useHTML: true,
                formatter: function () {
